...
const animateToBack = async (setSpringTarget) => {
  const duration = 200;
  setSpringTarget.start({
    xyrot: [-350, 0, 0],
    config: { duration: duration },
...
const deleteAnimated = async (setSpringTarget) => {
  const duration = 0;
  setSpringTarget.start({
    xyrot: [350, 0, 0],
    config: { duration: duration },
    ...
    React.useImperativeHandle(ref, () => ({
      async swipe(dir = "right") {
        ...
        if (dir === "right") {
          ...
          await deleteAnimated(setSpringTarget);
        }
          ...
          if (flickOnSwipe) {
            if (!preventSwipe.includes(dir)) {
              ...
              if (Math.abs(gesture.vx) > Math.abs(gesture.vy)) {
                if (Math.abs(gesture.vx) < 2) {
                  gesture.vx = gesture.vx > 0 ? 2 : -2;
                }
              } else {
                if (Math.abs(gesture.vy) < 2) {
                  gesture.vy = gesture.vy > 0 ? 2 : -2;
                }
              }
